...
//   c = a ^ 0x2000; // 1111 0001 0010 0011 toggle it again and it's back to the original
  ...
  cout << removeBits(0x12345678, 0xFFFFF0FF) << '\n'; // replace mask by a value so result=0x12345078
  cout << removeBits(0x12345678, 0xFFFF14FF) << '\n'; // replace mask by a value so result=0x12341478
  cout << setBits(0x12345678, 0x040B8101) << '\n'; // replace mask by a value so result=0x163FD779

  int v = 0x1234;
  cout << v << '\n';
  v = toggle(v, 0x200); // replace mask so v becomes 0x1034
  cout << v << '\n';
  v = toggle(v, 0x200); // mask should be the same as above, v should be back to 0x1234
  cout << v << '\n';

  cout << testBits(v, 0x1200) << '\n'; // test the bits labelled y: 000y 00y0 0011 0100 and return true if they are both set
  cout << testBits(v, 0x1400) << '\n'; // should return false since both these bits are NOT set.
